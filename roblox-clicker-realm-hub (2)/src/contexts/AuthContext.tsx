...
import { User, RegistrationKey } from '../types'; 
...
const getInitialUsers = (): User[] => {
  ...
  if (storedUsers) {
    ...
    // Ensure admin user exists if not found or if array is empty
    if (!parsedUsers.find((u: User) => u.id === initialAdminUser.id)) {
        return [...parsedUsers, initialAdminUser];
    }
    return parsedUsers;
  }
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const revokeAdminRole = async (userId: string) => {
    const user = users.find(u => u.id === userId);
    if (user) {
      await updateUserRoles(userId, ['user']); 
    }
  };
